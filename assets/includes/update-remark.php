...
try {
    ...
    $vehicleStatus  = $data['vehicle_status'] ?? null;

    // NEW FIELDS
    $vehicleNumber  = $data['vehicle_number'] ?? null;
    $flightNumber   = $data['flight_number'] ?? null;
    $airTicketImage = $data['air_ticket_image'] ?? null;
    ...
    // Existing air ticket
    $existingAirTicket = $row['air_ticket_image_url'] ?? null;

    // Upload NEW air ticket image if provided
    if (!empty($airTicketImage)) {

        $uploadDir = $_SERVER['DOCUMENT_ROOT'] . "/airport/uploaded/";

        if (!is_dir($uploadDir)) {
            mkdir($uploadDir, 0777, true);
        }

        if (preg_match('/^data:image\/(\w+);base64,/', $airTicketImage, $m)) {

            $bin = base64_decode(
                substr($airTicketImage, strpos($airTicketImage, ',') + 1)
            );
            ...
            if ($bin !== false) {
                ...
                file_put_contents($targetPath, $bin);

                $existingAirTicket =
                    '/airport/uploaded/' . $fileName;
            }
        }
    }

    // Save additional images (base64)
    $uploadedImages = [];
    ...
    if (!empty($editedEndDate)) {
        ...
        if (!$origTs) {
            throw new Exception('Original end date invalid');
        }

        if (!$editTs) {
            throw new Exception('Edited end date invalid');
        }
        ...
        $lateMinAfterGrace =
            (int)ceil(($editTs - $graceEndTs) / 60);

        $lateFeePercent =
            $surchargePct($lateMinAfterGrace);
